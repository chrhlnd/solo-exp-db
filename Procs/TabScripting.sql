...
DELIMITER $$
DROP FUNCTION IF EXISTS `?schema?`.`fScriptScrubColumnType` $$

CREATE FUNCTION `?schema?`.`fScriptScrubColumnType`(column_type_ VARCHAR(255)) RETURNS VARCHAR(255) CHARSET utf8
    DETERMINISTIC
BEGIN
	...
	IF LOCATE('MEDIUMTEXT',column_type_) > 0 THEN
		RETURN ' mtxt';
	END IF;
	
	IF LOCATE('LONGTEXT',column_type_) > 0 THEN
		RETURN ' ltxt';
	END IF;
	...
	IF LOCATE('DATE',column_type_) > 0 THEN
		return ' date';
	END IF;
...
DELIMITER $$
DROP FUNCTION IF EXISTS `?schema?`.`fScriptScriptIsNullable` $$

CREATE FUNCTION `?schema?`.`fScriptScriptIsNullable`(nullable_ VARCHAR(255)) RETURNS VARCHAR(255) CHARSET utf8
    DETERMINISTIC
BEGIN
...
DELIMITER $$
DROP FUNCTION IF EXISTS `?schema?`.`fScriptScrubDefault` $$

CREATE FUNCTION `?schema?`.`fScriptScrubDefault`(def_ VARCHAR(255), type_ VARCHAR(255)) RETURNS VARCHAR(255) CHARSET utf8
    DETERMINISTIC
BEGIN
...
DELIMITER $$
DROP FUNCTION IF EXISTS `?schema?`.`fScriptScrubExtra` $$

CREATE FUNCTION `?schema?`.`fScriptScrubExtra`(extra_ VARCHAR(255)) RETURNS VARCHAR(255) CHARSET utf8
    DETERMINISTIC
BEGIN
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `?schema?`.`pScriptTableToTab` $$
CREATE PROCEDURE `?schema?`.`pScriptTableToTab`(schema_ VARCHAR(100), table_ VARCHAR(100))
proc:BEGIN
	DECLARE OLDLEN INT;
	...
	INSERT INTO _lines VALUES (CONCAT("	,(charset utf8)"));
...
DELIMITER $$
DROP FUNCTION IF EXISTS `?schema?`.`fScriptColumnDataSelect` $$

CREATE FUNCTION `?schema?`.`fScriptColumnDataSelect`(coln_ VARCHAR(255), type_ VARCHAR(255)) RETURNS VARCHAR(255) CHARSET utf8
    DETERMINISTIC
BEGIN
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `?schema?`.`pScriptDataToTab` $$
CREATE PROCEDURE `?schema?`.`pScriptDataToTab`(schema_ VARCHAR(100), table_ VARCHAR(100))
proc:BEGIN
	DECLARE OLDLEN INT;
	...
	where c.table_schema = schema_ and
	      ...
	      (c.table_name LIKE '%tournament%' OR c.extra != "AUTO_INCREMENT");
